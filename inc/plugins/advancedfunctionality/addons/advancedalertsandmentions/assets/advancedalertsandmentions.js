(function () {
    ...
    function initSound() {
        ...
        // helper: если base заканчивается на /assets/ — дать вариант “на уровень выше”
        function baseWithoutAssets(b) {
            b = normBase(b);
            // .../assets/ -> .../
            return b.replace(/\/assets\/$/i, '/');
        }

        // соберём кандидатов баз
        var bases = [];
        if (typeof window.af_aam_asset_base === 'string' && window.af_aam_asset_base) {
            var b1 = normBase(window.af_aam_asset_base);
            var b2 = baseWithoutAssets(b1);

            bases.push(b1);
            if (b2 && b2 !== b1) bases.push(b2);
        }
        ...
        // список источников (по всем базам)
        var sources = [];
        bases.forEach(function (b) {
            sources.push(b + 'notification.mp3');
            sources.push(b + 'ping.mp3');
        });

        // 0) если есть <audio id="af_aam_sound"> — используем его, но тоже с фолбэками
        ...
        if (domAudio && typeof domAudio.play === 'function') {
            ...
            function tryNextOnDomAudio() {
                if (idx0 >= sources.length) {
                    if (afAamDebug) console.log('[AAM] initSound: no playable sources for <audio>');
                    return;
                }
                afAamSound.src = sources[idx0++];
                try { afAamSound.load(); } catch (e) {}
            }

            // если src пуст/левый — стартуем с наших кандидатов
            if (!afAamSound.src || afAamSound.src === window.location.href) {
                tryNextOnDomAudio();
            }
            ...
            if (afAamDebug) console.log('[AAM] initSound: using existing <audio>, sources=', sources);
            return;
        }

        // 1) fallback: создаём Audio вручную
        try {
            afAamSound = new Audio();
            afAamSound.preload = 'auto';
            afAamSound.volume = 1.0;
            afAamSound.muted = false;
            ...
            function tryNext() {
                ...
                if (idx >= sources.length) {
                    if (afAamDebug) console.log('[AAM] initSound: no playable sources, giving up');
                    return;
                }
                ...
                if (afAamDebug) console.log('[AAM] initSound: try src =', src);
            }
        ...
        // Firefox иногда не играет звук у “висячих” Audio — прикрепим к DOM
        ...
        if (afAamDebug) console.log('[AAM] initSound ok=', !!afAamSound, 'sources=', sources);
    }
    ...
    function afAamFixPidAnchor(url) {
        try {
            url = String(url || '').trim();
            if (!url) return url;

            // если якорь уже есть — не трогаем
            if (url.indexOf('#') !== -1) return url;

            // если в URL есть pid=123 — добавим #pid123
            var m = url.match(/[?&]pid=([0-9]+)/i);
            if (m && m[1]) {
                return url + '#pid' + String(m[1]).replace(/[^0-9]/g, '');
            }
        } catch (e) {}
        return url;
    }
    ...
    /**
     * Фикс клика по уведомлению в модалке:
     * - гарантирует навигацию
     * - по возможности помечает как прочитанное через sendBeacon (без ожидания ответа)
     */
    function afAamBindAlertRowNavigation() {
        // в capture, чтобы сработать даже если кто-то делает stopPropagation()
        document.addEventListener('click', function (e) {
            try {
                var modal = document.getElementById('af_aam_modal');
                if (!modal) return;
                ...
                // ищем строку алерта
                var row = t.closest ? t.closest('[data-alert-id]') : null;
                if (!row) return;

                // кнопки действий (их НЕ перехватываем)
                var actionBtn = t.closest ? t.closest('.markReadAlertButton, .markUnreadAlertButton, .deleteAlertButton') : null;
                if (actionBtn) return;

                // ищем ссылку, по которой “должно” перейти
                var a = t.closest ? t.closest('a') : null;
                if (!a) return;

                // если пользователь хочет открыть в новой вкладке — не мешаем
                if (e.button !== 0 || e.ctrlKey || e.metaKey || e.shiftKey || e.altKey) return;

                // достаём URL максимально гибко
                var href =
                    (a.getAttribute && (a.getAttribute('data-href') || a.getAttribute('data-url'))) ||
                ...
                // мусорные href — не трогаем
                var low = href.toLowerCase();
                if (low === '#' || low.indexOf('javascript:') === 0 || low.indexOf('void(0)') !== -1) return;

                // теперь мы берём управление навигацией на себя
                e.preventDefault();

                // помечаем прочитанным “по пути” (не блокируем переход)
                var id = getAlertIdFromElement(row) || getAlertIdFromElement(a);
                if (id && window.my_post_key) {
                    afAamSendBeacon({
                        action: 'af_aam_api',
                        op: 'mark_read',
                        id: id,
                ...
                // небольшой бонус: если есть pid=, но нет #pid — добавим
                href = afAamFixPidAnchor(href);

                // навигация
                window.location.href = href;
            } catch (err) {
                // молча, чтобы не убить клики вообще
            }
        }, true);
    }

    /**
    * Фикс “клик по нику/кнопке упоминания”: после вставки упоминания
    * прокручиваем и фокусим редактор.
    *
    * ВАЖНО: мы ничего не отменяем и не вставляем сами — только прыгаем к редактору.
    */
    function afAamBindJumpToEditorOnMentionClicks() {
        document.addEventListener('click', function (e) {
            try {
                var t = e.target;
                if (!t) return;
                ...
                // ник в постбите и кнопка "@"
                var hit = t.closest ? t.closest('a.af-aam-mention-user, a.af-aam-mention-button') : null;
                if (!hit) return;

                // если на странице нет редактора — не дёргаемся
                var hasEditor =
                    document.querySelector('textarea[name="message"]') ||
                    document.getElementById('message') ||
                    document.getElementById('message_new') ||
                ...
                // даём твоему текущему обработчику вставки упоминания отработать первым
                setTimeout(function () {
                    afAamJumpToReplyAndFocus();
                }, 0);
            } catch (err) {}
        }, true);
    }

    // Вызови один раз (можно прямо внизу файла, внутри IIFE)
    afAamBindAlertRowNavigation();
    afAamBindJumpToEditorOnMentionClicks();

...
function af_advancedrules_pre_output(&$page = '')
{
    ...
    // Уже вставляли нашим способом — выходим
    if (strpos($page, '<!--af_advancedrules_nav-->') !== false) {
        return $page;
    }

    // Если ссылка уже есть где угодно на странице — НЕ ТРОГАЕМ и НЕ ПЫТАЕМСЯ "пометить" URL
    // (иначе ломаем href и получаем %3C!--...--%3E)
    if (stripos($page, 'misc.php?action=advancedrules') !== false) {
        return $page;
    }

    // По умолчанию — стандартный UL top_links
    $selector = trim((string)($mybb->settings['af_advancedrules_nav_where'] ?? 'ul.menu.top_links'));
    if ($selector === '') {
        $selector = 'ul.menu.top_links';
    }

    // Сейчас поддерживаем дефолтный контейнер. (Если тебе нужно больше — расширим позже, но сейчас чиним конкретную поломку.)
    if (stripos($selector, 'ul.menu.top_links') === false) {
        return $page;
    }

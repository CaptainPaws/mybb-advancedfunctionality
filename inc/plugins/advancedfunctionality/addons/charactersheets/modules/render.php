...
function af_charactersheets_get_render_path_override(): string
{
    global $mybb;

    $raw = strtolower(trim((string)($mybb->settings['af_charactersheets_render_path_mode'] ?? 'auto')));
    if (!in_array($raw, ['auto', 'dnd', 'arpg'], true)) {
        return 'auto';
    }
...
function af_charactersheets_resolve_render_path(array $sheet, array $sheet_view, array $atf_index = []): string
{
    $override = af_charactersheets_get_render_path_override();
    if ($override === 'dnd' || $override === 'arpg') {
        return $override;
    }
...
function af_charactersheets_resolve_arpg_stat_value(string $key, array $sheet_view)
{
    ...
    $path_map = [
        'hp' => 'mechanics.hp_total',
        'atk' => 'mechanics.damage_bonus',
        'attack' => 'mechanics.damage_bonus',
        'def' => 'mechanics.ac_total',
        'defense' => 'mechanics.ac_total',
        'crit_rate' => 'character_computed_state.fixed_bonuses.crit_rate',
        'crit_dmg' => 'character_computed_state.fixed_bonuses.crit_dmg',
        'crit_damage' => 'character_computed_state.fixed_bonuses.crit_dmg',
        'bonus_damage' => 'mechanics.damage_total',
        'mastery' => 'character_computed_state.resources.mastery',
        'regen' => 'character_computed_state.resources.regen',
        'speed' => 'mechanics.speed_total',
        'humanity' => 'mechanics.humanity_total',
    ];

    $path = $path_map[$key] ?? ('character_computed_state.resources.' . $key);
    $value = af_charactersheets_deep_get($sheet_view, $path, null);
    if ($value !== null) {
        return $value;
    }

    return af_charactersheets_deep_get($sheet_view, 'mechanics.' . $key, null);
}

function af_charactersheets_deep_get(array $source, string $path, $fallback = null)
{
    if ($path === '') {
        return $fallback;
    }
...
function af_charactersheets_extract_character_contract_payload(array $entry): array
{
    if (function_exists('af_kb_extract_character_contract')) {
        $extracted = af_kb_extract_character_contract($entry);
        if (is_array($extracted)) {
            return [
                'profile' => (array)($extracted['profile'] ?? []),
                'stats' => (array)($extracted['stats'] ?? []),
                'abilities' => (array)($extracted['abilities'] ?? []),
                'meta' => (array)($extracted['meta'] ?? []),
            ];
        }
    }
    ...
    if (!empty($entry['data_json'])) {
        $rules = af_charactersheets_json_decode((string)$entry['data_json']);
    }
    ...
    return [
        ...
        'meta' => (array)($rules['character_meta'] ?? []),
    ];
}

function af_charactersheets_resolve_character_kb_entry(int $tid, int $uid, array $accept_row = []): array
{
    global $db;

    $empty = [
        'entry' => [],
        'payload' => ['profile' => [], 'stats' => [], 'abilities' => [], 'meta' => []],
        'source' => 'none',
    ];
    ...
    $normalize = static function (array $entry, string $source): array {
        return [
            'entry' => $entry,
            'payload' => af_charactersheets_extract_character_contract_payload($entry),
            'source' => $source,
        ];
    };

    if (function_exists('af_cwf_has_linked_kb_character')) {
        $linked = af_cwf_has_linked_kb_character($tid, $accept_row);
        if (!empty($linked['ok']) && !empty($linked['entry'])) {
            return $normalize((array)$linked['entry'], (string)$linked['source']);
        }
    }

    $oc_key = 'oc-' . $tid;
    $oc_entry = af_charactersheets_kb_get_entry('character', $oc_key);
    if (!empty($oc_entry['id'])) {
        return $normalize((array)$oc_entry, 'character.key:oc-tid');
    }

    $q = $db->simple_select(
        'af_kb_entries',
        '*',
        "type='character' AND active=1",
    ...
    while ($row = $db->fetch_array($q)) {
        ...
        $payload = af_charactersheets_extract_character_contract_payload($row);
        $meta = (array)($payload['meta'] ?? []);
        $source_tid = (int)($meta['source_tid'] ?? 0);
        if ($source_tid > 0 && $source_tid === $tid) {
            return [
                'entry' => $row,
                'payload' => $payload,
                'source' => 'character_meta.source_tid',
            ];
        }
    }
...
function af_charactersheets_resolve_arpg_primary_stat_specs(array $sheet_view): array
{
    $paths = [
...
{
    $result = [];
    foreach (af_charactersheets_resolve_arpg_primary_stat_specs($sheet_view) as $spec) {
        $key = '';
        $label = '';
        $path = '';
        ...
        if (is_string($spec)) {
            ...
            $path = trim((string)($spec['path'] ?? $spec['value_path'] ?? ''));
        }
        ...
        $value = $path !== '' ? af_charactersheets_deep_get($sheet_view, $path, null) : null;
        if ($value === null) {
            $value = af_charactersheets_resolve_arpg_stat_value($key, $sheet_view);
        }
        if ($value === null || $value === '') {
            continue;
        }
...
function af_charactersheets_arpg_format_kb_item(string $kb_type, string $kb_key, int $qty = 1): array
{
...
function af_charactersheets_arpg_collect_equipment_items(array $build): array
{
    ...
    foreach ($slots as $slot_code => $slot_item) {
        ...
        $kb_type = af_charactersheets_get_inventory_item_type($slot_item);
        $kb_key = af_charactersheets_get_inventory_item_key($slot_item);
        if ($kb_type === '' || $kb_key === '') {
            continue;
        }
        $item = af_charactersheets_arpg_format_kb_item($kb_type, $kb_key, 1);
        $item['slot'] = (string)$slot_code;
        $result[] = $item;
    }
    return $result;
}
...
function af_charactersheets_arpg_collect_abilities(array $build, array $sheet_view): array
{
    ...
    foreach ((array)($build['abilities']['owned'] ?? []) as $ability_item) {
        if (!is_array($ability_item)) {
            continue;
        }
        $kb_type = af_charactersheets_get_inventory_item_type($ability_item);
        $kb_key = af_charactersheets_get_inventory_item_key($ability_item);
        if ($kb_type === '' || $kb_key === '') {
            continue;
        }
        $result[] = af_charactersheets_arpg_format_kb_item($kb_type, $kb_key, 1);
    }
    ...
    foreach ((array)($sheet_view['character_computed_state']['passive_abilities'] ?? []) as $passive) {
        ...
        $label = trim((string)($passive['key'] ?? $passive['kind'] ?? ''));
        if ($label === '') {
            continue;
        }
        $result[] = [
...
{
    if (!$items) {
        return '<div class="af-cs-muted">' . htmlspecialchars_uni($empty_label) . '</div>';
    }
...
function af_charactersheets_arpg_format_value($value, string $suffix = ''): string
{
    if ($value === null || $value === '') {
        return '—';
    }
    if (is_numeric($value)) {
        ...
        if (floor($number) === $number) {
            return number_format((int)$number, 0, '.', ' ') . $suffix;
        }
...
function af_charactersheets_arpg_pick_stat(array $sheet_view, array $paths, string $fallback = '—'): string
{
    foreach ($paths as $path) {
        $value = af_charactersheets_deep_get($sheet_view, $path, null);
        if ($value !== null && $value !== '') {
            return af_charactersheets_arpg_format_value($value);
        }
    }
    return $fallback;
}
...
function af_charactersheets_arpg_extract_entry_rules(array $entry): array
{
    if (!$entry) {
        return [];
    }
...
function af_charactersheets_arpg_resolve_kb_entry_flexible(string $type, string $rawValue): array
{
    ...
    if ($type === '' || $rawValue === '') {
        return ['entry' => [], 'resolved_key' => '', 'resolved_by' => 'empty'];
    }

    $direct = af_charactersheets_kb_get_entry($type, $rawValue);
    if (!empty($direct['id'])) {
        return ['entry' => $direct, 'resolved_key' => (string)($direct['key'] ?? $rawValue), 'resolved_by' => 'direct.key'];
    }

    if (ctype_digit($rawValue) && is_object($db) && $db->table_exists('af_kb_entries')) {
        $byId = $db->fetch_array($db->simple_select(
            'af_kb_entries',
            '*',
            'id=' . (int)$rawValue . " AND type='" . $db->escape_string($type) . "' AND active=1",
            ['limit' => 1]
        ));
        if (is_array($byId) && !empty($byId['id'])) {
            return ['entry' => $byId, 'resolved_key' => (string)($byId['key'] ?? ''), 'resolved_by' => 'direct.id'];
        }
    }

    $normalizedRaw = function_exists('my_strtolower') ? my_strtolower($rawValue) : strtolower($rawValue);
    foreach (af_charactersheets_get_kb_entries_by_type($type) as $entry) {
        $entryKey = trim((string)($entry['key'] ?? ''));
        $entryTitle = trim(af_charactersheets_kb_pick_text($entry, 'title'));
        $entryKeyNorm = function_exists('my_strtolower') ? my_strtolower($entryKey) : strtolower($entryKey);
        $entryTitleNorm = function_exists('my_strtolower') ? my_strtolower($entryTitle) : strtolower($entryTitle);
        if ($entryKeyNorm === $normalizedRaw || $entryTitleNorm === $normalizedRaw) {
            return ['entry' => $entry, 'resolved_key' => $entryKey, 'resolved_by' => $entryKeyNorm === $normalizedRaw ? 'scan.key' : 'scan.title'];
        }
    }

    return ['entry' => [], 'resolved_key' => '', 'resolved_by' => 'not_found'];
}

function af_charactersheets_arpg_numeric_rule(array $rules, string $key): float
{
...
function af_charactersheets_arpg_build_stats_from_kb(array $originRules, array $archetypeRules, int $level, array $originVariantRules = [], array $equipmentRuleSources = []): array
{
    ...
    $stats = [
        'character_hp' => $sum('hp_base') + $lvlScale * $sum('hp_per_level'),
        'character_defense' => $sum('defense_base') + $lvlScale * $sum('defense_per_level'),
        'character_attack_power' => $sum('attack_power_base') + $lvlScale * $sum('attack_power_per_level'),
        'character_crit_damage' => $sum('crit_damage_base'),
        'character_elemental_mastery' => $sum('elemental_mastery_base') + $lvlScale * $sum('elemental_mastery_per_level'),
...
function af_charactersheets_arpg_collect_weapon_data(array $build): array
{
    ...
    foreach ($weapon_slots as $slot_code) {
        $slot_item = (array)($slots[$slot_code] ?? []);
        $kb_type = af_charactersheets_get_inventory_item_type($slot_item);
        $kb_key = af_charactersheets_get_inventory_item_key($slot_item);
        if ($kb_type === '' || $kb_key === '') {
            continue;
        ...
        $meta = af_charactersheets_json_decode((string)($entry['meta_json'] ?? ''));
        $rules = (array)($meta['rules'] ?? []);
        $weapon = (array)($rules['weapon'] ?? []);
        $ui = (array)($rules['ui'] ?? []);

        $variant = [
            'slot' => $slot_code,
            'title' => $title,
            'level' => (string)($weapon['level'] ?? $rules['level'] ?? ''),
            'base_attack' => af_charactersheets_arpg_format_value($weapon['base_attack'] ?? $weapon['attack'] ?? $rules['attack'] ?? null),
            'attack_percent' => af_charactersheets_arpg_format_value($weapon['damage_bonus'] ?? $rules['damage_bonus'] ?? null, '%'),
            'image' => trim((string)($ui['image'] ?? $ui['icon'] ?? '')),
            'type' => $kb_type,
            'key' => $kb_key,
        ];
        $variants[] = $variant;
...
function af_charactersheets_arpg_collect_artifacts_data(array $build): array
{
    ...
    $appendArtifact = static function (array $item, string $source) use (&$result): void {
        ...
        $meta = af_charactersheets_json_decode((string)($entry['meta_json'] ?? ''));
        $rules = (array)($meta['rules'] ?? []);
        $itemRules = (array)($rules['item'] ?? []);
        $kind = strtolower(trim((string)($itemRules['item_kind'] ?? $itemRules['kind'] ?? '')));
        if ($kind !== 'artifact') {
            return;
        }
        ...
        $result[] = [
            'title' => $title,
            'level' => (string)($itemRules['level'] ?? $rules['level'] ?? ''),
            'main' => af_charactersheets_arpg_format_value($itemRules['main_stat'] ?? $rules['main_stat'] ?? ''),
            'sub' => af_charactersheets_arpg_format_value($itemRules['sub_stat'] ?? $rules['sub_stat'] ?? ''),
            'source' => $source,
        ];
    };

    foreach ($slots as $slotCode => $slotItem) {
        if (!is_array($slotItem)) {
            continue;
        }
...
function af_charactersheets_build_arpg_view_model(array $sheet, array $sheet_view, array $atf_index, array $build = [], array $character_source = []): array
{
    ...
    $identityParts = [];
    foreach ([
        (string)($character_profile['character_identity'] ?? $character_profile['character_origin'] ?? ($has_kb_character ? '' : af_charactersheets_pick_field_value($atf_index, ['character_identity', 'identity', 'origin']))),
        (string)($character_profile['character_class'] ?? ''),
        (string)($character_profile['character_faction'] ?? ''),
        (string)($character_profile['character_path'] ?? ($has_kb_character ? '' : af_charactersheets_pick_field_value($atf_index, ['character_path', 'path']))),
    ] as $part) {
    ...
    $rank = trim((string)($character_profile['character_rank'] ?? ($has_kb_character ? '' : af_charactersheets_pick_field_value($atf_index, ['rank', 'character_rank']))));
    $path = trim((string)($character_profile['character_path'] ?? ($has_kb_character ? '' : af_charactersheets_pick_field_value($atf_index, ['path', 'character_path']))));
    $level = (int)($sheet_view['level'] ?? 1);
    $rawOrigin = trim((string)($character_profile['character_origin'] ?? ($has_kb_character ? '' : af_charactersheets_pick_field_value($atf_index, ['character_origin', 'character_race', 'race']))));
    $rawOriginVariant = trim((string)($character_profile['character_origin_variant'] ?? ($has_kb_character ? '' : af_charactersheets_pick_field_value($atf_index, ['character_origin_variant', 'origin_variant']))));
    $rawArchetype = trim((string)($character_profile['character_archetype'] ?? $character_profile['character_class'] ?? ($has_kb_character ? '' : af_charactersheets_pick_field_value($atf_index, ['character_archetype', 'character_class', 'archetype', 'class']))));
    ...
    $rawElement = trim((string)($character_profile['character_element'] ?? ($has_kb_character ? '' : af_charactersheets_pick_field_value($atf_index, ['character_element', 'element']))));
    ...
    return [
        ...
        'atf' => [
            ...
            'class' => (string)($character_profile['character_class'] ?? $character_profile['character_archetype'] ?? ($has_kb_character ? '' : af_charactersheets_pick_field_value($atf_index, ['character_class', 'character_archetype', 'class']))),
            'theme' => (string)($character_profile['character_faction'] ?? ($has_kb_character ? '' : af_charactersheets_pick_field_value($atf_index, ['character_theme', 'character_themes', 'theme']))),
        ],
        'atf_profile' => [
            'character_app' => (string)($character_profile['character_app'] ?? ($has_kb_character ? '' : af_charactersheets_pick_field_value($atf_index, ['character_app', 'character_about', 'character_bio', 'character_description', 'app']))),
            'character_prototype' => (string)($character_profile['character_prototype'] ?? ($has_kb_character ? '' : af_charactersheets_pick_field_value($atf_index, ['character_prototype', 'prototype']))),
            'character_nicknames' => (string)($character_profile['character_nicknames'] ?? ($has_kb_character ? '' : af_charactersheets_pick_field_value($atf_index, ['character_nicknames', 'character_nickname', 'nickname']))),
            'character_age' => (string)($character_profile['character_age'] ?? ($has_kb_character ? '' : af_charactersheets_pick_field_value($atf_index, ['character_age', 'age']))),
            'character_origin' => (string)($character_profile['character_origin'] ?? ($has_kb_character ? '' : af_charactersheets_pick_field_value($atf_index, ['character_origin']))),
            'character_origin_variant' => (string)($character_profile['character_origin_variant'] ?? ($has_kb_character ? '' : af_charactersheets_pick_field_value($atf_index, ['character_origin_variant', 'origin_variant']))),
        ],
        'arpg_sources' => [
            'origin' => [
                'raw' => $rawOrigin,
            ...
            'archetype' => [
                'raw' => $rawArchetype,
                'resolved_key' => (string)($archetypeResolved['resolved_key'] ?? ''),
                'resolved_by' => (string)($archetypeResolved['resolved_by'] ?? ''),
...
function af_charactersheets_arpg_render_stats_panel_html(array $stats): string
{
    if (!$stats) {
        return '<div class="af-cs-muted">Нет характеристик</div>';
    }

    $html = '';
    foreach ($stats as $row) {
        $html .= '<div class="af-cs-arpg-stat-row">'
            . '<span class="af-cs-arpg-stat-label">' . htmlspecialchars_uni((string)($row['label'] ?? '—')) . '</span>'
            . '<strong class="af-cs-arpg-stat-value">' . htmlspecialchars_uni((string)($row['value'] ?? '—')) . '</strong>'
            . '</div>';
    }
    return $html;
}

function af_charactersheets_arpg_render_weapon_html(array $weapon): string
{
    $active = (array)($weapon['active'] ?? []);
    if (!$active) {
        return '<div class="af-cs-muted">Оружие не экипировано</div>';
    }

    $image = trim((string)($active['image'] ?? ''));
    $title = htmlspecialchars_uni((string)($active['title'] ?? '—'));
    $level = trim((string)($active['level'] ?? ''));
    $baseAttack = htmlspecialchars_uni((string)($active['base_attack'] ?? '—'));
    $attackPercent = htmlspecialchars_uni((string)($active['attack_percent'] ?? '—'));
    $variants = (array)($weapon['variants'] ?? []);
    $variantsHtml = '';
    foreach ($variants as $variant) {
        $variantsHtml .= '<span class="af-cs-chip">' . htmlspecialchars_uni((string)($variant['title'] ?? '—')) . '</span>';
    }

    return '<article class="af-cs-arpg-weapon-card">'
        . ($image !== '' ? '<div class="af-cs-arpg-weapon-card__image"><img src="' . htmlspecialchars_uni($image) . '" alt="' . $title . '" loading="lazy" /></div>' : '')
        . '<div class="af-cs-arpg-weapon-card__content">'
        . '<h3>' . $title . ($level !== '' ? '<span class="af-cs-chip">Ур. ' . htmlspecialchars_uni($level) . '</span>' : '') . '</h3>'
        . '<div class="af-cs-arpg-weapon-card__meta">Базовая атака: <strong>' . $baseAttack . '</strong></div>'
        . '<div class="af-cs-arpg-weapon-card__meta">Сила атаки: <strong>' . $attackPercent . '</strong></div>'
        . '<div class="af-cs-arpg-weapon-card__variants"><span class="af-cs-muted">Смена оружия:</span>' . $variantsHtml . '</div>'
        . '</div>'
        . '</article>';
}

function af_charactersheets_arpg_render_artifacts_html(array $artifacts): string
{
    if (!$artifacts) {
        return '<div class="af-cs-muted">Артефакты не надеты</div>';
    }
    $cards = '';
    foreach ($artifacts as $artifact) {
        $cards .= '<article class="af-cs-arpg-artifact-card">'
            . '<h4>' . htmlspecialchars_uni((string)($artifact['title'] ?? '—')) . '</h4>'
            . '<div class="af-cs-muted">' . htmlspecialchars_uni((string)($artifact['source'] ?? '')) . '</div>'
            . '<div>Осн. атрибут: <strong>' . htmlspecialchars_uni((string)($artifact['main'] ?? '—')) . '</strong></div>'
            . '<div>Доп. атрибут: <strong>' . htmlspecialchars_uni((string)($artifact['sub'] ?? '—')) . '</strong></div>'
            . '</article>';
    }
    return '<div class="af-cs-arpg-artifacts-grid">' . $cards . '</div>';
}

function af_charactersheets_arpg_render_abilities_group_html(array $items, string $emptyLabel): string
{
    if (!$items) {
        return '<div class="af-cs-muted">' . htmlspecialchars_uni($emptyLabel) . '</div>';
    }

    $html = '';
    foreach ($items as $item) {
        $icon = trim((string)($item['icon'] ?? $item['icon_url'] ?? $item['image'] ?? ''));
        $labels = is_array($item['labels'] ?? null) ? (array)$item['labels'] : [];
        $labelsHtml = '';
        foreach ($labels as $label) {
            ...
            $labelsHtml .= '<span class="af-cs-chip">'
                . ($labelName !== '' ? htmlspecialchars_uni($labelName) . ': ' : '')
...
function af_charactersheets_arpg_render_description_html(array $sheet_arpg_vm): string
{
    ...
    return '<section class="af-cs-arpg-panel"><h2>Описание персонажа</h2><div class="af-cs-arpg-main-about"><div>' . $aboutHtml . '</div></div></section>';
}
...
function af_charactersheets_build_sheet_inner_html(string $slug): string
{
    ...
    $character_name_ru = trim((string)($character_profile['character_name_ru'] ?? ''));
    if ($character_name_ru === '') {
        $character_name_ru = af_charactersheets_pick_field_value($atf_index, ['character_name_ru']);
    }
    $character_nicknames = trim((string)($character_profile['character_nicknames'] ?? ''));
    if ($character_nicknames === '') {
    ...
    $sheet_portrait_url = trim((string)($character_profile['character_pic'] ?? ''));
    if ($sheet_portrait_url === '') {
    ...
    if ($is_arpg_sheet) {
        ...
        $sheet_arpg_hp = htmlspecialchars_uni((string)($sheet_arpg_combat['hp_total'] ?? ($character_stats['character_hp'] ?? 0)));
        $sheet_arpg_def = htmlspecialchars_uni((string)($sheet_arpg_combat['def_total'] ?? ($character_stats['character_defense'] ?? 0)));
...
function af_charactersheets_build_info_table_html(array $index, array $sheet_view = [], array $character_source = []): string
{
    global $mybb;

    $character_profile = (array)(($character_source['payload'] ?? [])['profile'] ?? []);
    $age = af_charactersheets_pick_field_value($index, ['character_age', 'age']);
    $gender = trim((string)($character_profile['character_gen'] ?? ''));
    if ($gender === '') {
        $gender = af_charactersheets_pick_field_value($index, ['character_gen', 'character_gender', 'gender']);
    }
    $race_key = trim((string)($character_profile['character_race'] ?? ''));
    if ($race_key === '') {
    ...
    $class_key = trim((string)($character_profile['character_class'] ?? $character_profile['character_archetype'] ?? ''));
    if ($class_key === '') {
        $class_key = af_charactersheets_pick_field_value($index, ['character_class', 'character_archetype', 'class'], false);
    }
    $theme_key = af_charactersheets_pick_field_value($index, ['character_themes', 'character_theme', 'theme'], false);
    $race_label = $race_key !== '' ? $race_key : af_charactersheets_pick_field_value($index, ['character_race', 'race'], true);
    $race_variant_label = af_charactersheets_pick_field_value($index, ['character_race_variant', 'race_variant', 'racevariant'], true);
    $class_label = $class_key !== '' ? $class_key : af_charactersheets_pick_field_value($index, ['character_class', 'character_archetype', 'class'], true);
...
function af_charactersheets_build_bonus_html(array $index, array $sheet_view = []): string
{
    ...
    $columns = [];
    $bonus_from_entry = static function (array $entry): string {
        if (empty($entry)) {
            return '';
        }

        $html = trim((string)af_charactersheets_kb_get_block_html($entry, 'bonuses'));
        if ($html === '') {
            return '';
        }
    ...
    foreach ($mapping as $fieldName => $data) {
        ...
        $text_html = $bonus_from_entry($entry);
...
function af_charactersheets_render_catalog_page(): void
{
    ...
    foreach ($rows as $row) {
        ...
        $class = af_charactersheets_pick_field_value($index, ['character_class', 'character_archetype', 'class']);

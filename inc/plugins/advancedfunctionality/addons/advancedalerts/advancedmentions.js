(function () {
    ...
    if (window.afAdvancedMentionsConfig && typeof window.afAdvancedMentionsConfig === 'object') {
        var phpCfg = window.afAdvancedMentionsConfig;

        if (typeof phpCfg.minChars === 'number') {
            cfg.minChars = phpCfg.minChars;
        }
        if (typeof phpCfg.clickInsert === 'boolean') {
            cfg.clickInsert = phpCfg.clickInsert;
        }
        if (typeof phpCfg.suggestUrl === 'string') {
            cfg.suggestUrl = phpCfg.suggestUrl;
        }
    }
    ...
    function handleUsernameClick(e) {
        ...
        // === Клик по нику-профилю (ТОЛЬКО внутри .post_author) ===
        if (!cfg.clickInsert) {
            return;
        }

        // Ограничиваем область: только блок автора поста
        if (!target.closest || !target.closest('.post_author')) {
            // Всё остальное — обычный переход по ссылке
            return;
        }

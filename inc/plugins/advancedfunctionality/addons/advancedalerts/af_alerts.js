(function () {
  ...
  // ==== ДВА возможных эндпоинта API: сначала пробуем xmlhttp.php, если там пусто — падаем на misc.php ====
  var apiCandidates = [
    //'xmlhttp.php?action=af_alerts_api',
    'misc.php?action=af_alerts_api'
  ];
  var apiIndex = 0;

  function currentApi() {
    return apiCandidates[apiIndex];
  }
  ...
  /**
   * Универсальный POST в API с fallback:
   * 1) Пытаемся стукнуться в текущий эндпоинт (xmlhttp.php или misc.php).
   * 2) Если статус 200, но тело вообще пустое — переключаемся на второй эндпоинт и пробуем ещё раз.
   */
  function post(op, data, _attempt) {
    data = data || {};
    data.op = op;
    ...
    $.ajax({
      ...
      var body = '';
      try {
        body = (xhr && xhr.responseText ? String(xhr.responseText).trim() : '');
      } catch (e) {}

      // Если получили 200, но тело пустое — это как раз твой AF Alerts API empty response 200.
      // Пробуем один раз переключиться на запасной эндпоинт.
      if (!_attempt && xhr && xhr.status === 200 && !body) {
        console.warn(
          'AF Alerts API empty response from',
          url,
          'op =', op,
          '— switching to fallback endpoint'
        );
        apiIndex = (apiIndex + 1) % apiCandidates.length; // переключаемся xmlhttp <-> misc
        post(op, data, true).done(dfd.resolve).fail(dfd.reject);
        return;
      }
  ...
  $(document).on('click', '[data-afaa-open-prefs]', function (e) {
    e.preventDefault();
    switchView('prefs');
    ensureTypesLoaded().always(function () {
      // если types ок — prefs уже отрисованы
    });
  });

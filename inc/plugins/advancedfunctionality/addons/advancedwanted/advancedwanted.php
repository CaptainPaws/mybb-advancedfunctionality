...
function af_wanted_field_control(array $f,$value,array $all=[]): string {
 ...
 $htmlType=in_array($type,['url','image'],true)?'url':($type==='number'?'number':'text'); return '<input type="'.$htmlType.'" name="fields['.$k.']" value="'.af_wanted_h((string)$v).'"'.$req.($type==='image'?' placeholder="https://example.com/image.jpg"':'').'>';
}

...
function af_wanted_entry(int $id): array { global $db; if($id<1)return []; return (array)$db->fetch_array($db->simple_select(AF_WANTED_ENTRIES,'*','id='.$id,['limit'=>1])); }
...
function af_wanted_validate(array $fields,array $raw): array {
 ...
 if($f['type']==='url'&&$v!==''&&!filter_var($v,FILTER_VALIDATE_URL))$errors[]='Некорректный URL в поле «'.$f['title'].'».';
...
function af_wanted_field_control(array $f,$value,array $all=[]): string {
 ...
 if($type==='multi'){ $vals=is_array($v)?$v:(json_decode((string)$v,true)?:[]);$h='<select multiple name="fields['.$k.'][]">';foreach(af_wanted_options($f,$all) as $ok=>$ol)$h.='<option value="'.af_wanted_h($ok).'"'.(in_array($ok,$vals,true)?' selected':'').'>'.af_wanted_h($ol).'</option>';return $h.'</select>'; }
 if($type==='checkbox')return '<input type="checkbox" name="fields['.$k.']" value="1"'.((string)$v==='1'?' checked':'').'>';
 $htmlType=in_array($type,['url','number'],true)?$type:'text'; return '<input type="'.$htmlType.'" name="fields['.$k.']" value="'.af_wanted_h((string)$v).'"'.$req.'>';
}
...
function af_wanted_catalog(): string {
 ...
 $query=http_build_query($params);$h='<div class="af-wanted-tabs"><a class="button" href="wanted.php?tab=active">Активный поиск</a><a class="button" href="wanted.php?tab=archive">Архив</a></div>';if(af_wanted_can('create'))$h.='<p><a class="button" href="wanted.php?action=create">Создать нужного персонажа</a></p>';
 $h.='<form class="af-wanted-filters" method="get"><input type="hidden" name="tab" value="'.$tab.'"><label>Поиск <input name="search" value="'.af_wanted_h($search).'"></label>';if($tab==='active'){$h.='<label>Статус <select name="status"><option value="">Все</option>';foreach(['open'=>'Свободные','reserved'=>'Придержанные','application'=>'С анкетой'] as $k=>$l)$h.='<option value="'.$k.'"'.($status===$k?' selected':'').'>'.$l.'</option>';$h.='</select></label>';}foreach($filters as [$f,$v]){$h.='<label>'.af_wanted_h($f['title']).' <select name="'.af_wanted_h($f['field_key']).'"><option value="">Все</option>';foreach(af_wanted_options($f,$mybb->input) as $k=>$l)$h.='<option value="'.af_wanted_h($k).'"'.($v===$k?' selected':'').'>'.af_wanted_h($l).'</option>';$h.='</select></label>';}$h.='<label>Сортировка <select name="sort"><option value="newest">Новые сначала</option><option value="oldest"'.($sort==='oldest'?' selected':'').'>Старые сначала</option></select></label><button type="submit">Применить</button><a href="wanted.php?tab='.$tab.'">Сбросить</a></form><div class="af-wanted-grid">';
 foreach($entries as $e){$ev=$values[$e['id']]??[];$title='Wanted #'.(int)$e['id'];$image='';$details='';foreach($fields as $f){$v=$ev[$f['id']]??'';if($v==='')continue;if(in_array($f['field_key'],['title','name','character_name'],true))$title=af_wanted_h($v);if($f['type']==='image')$image='<img src="'.af_wanted_h($v).'" alt="">';if(!empty($f['settings']['show_card'])&&!in_array($f['type'],['textarea','image'],true))$details.='<span><b>'.af_wanted_h($f['title']).':</b> '.af_wanted_h(af_wanted_label($f,$v)).'</span>';}$h.='<article class="af-wanted-card">'.$image.'<h2>'.$title.'</h2><small>Wanted #'.(int)$e['id'].' · '.af_wanted_status_label($e['status']).'</small><div>'.$details.'</div><p>Автор: '.build_profile_link(af_wanted_h((string)$e['username']),(int)$e['author_uid']).'</p>';if($e['status']==='reserved')$h.='<p>Придержано пользователем '.build_profile_link(af_wanted_h((string)$e['reserved_name']),(int)$e['reserved_by_uid']).'</p>';if($e['status']==='archived'&&!empty($e['accepted_uid']))$h.='<p>Игрок: '.build_profile_link(af_wanted_h((string)$e['accepted_name']),(int)$e['accepted_uid']).'</p>';$h.='<a class="button" href="wanted.php?action=view&id='.(int)$e['id'].'">Подробнее</a>'.af_wanted_actions($e).'</article>';}$h.='</div>';if(function_exists('multipage'))$h.=multipage($count,$per,$page,'wanted.php?'.$query);return $h;
}

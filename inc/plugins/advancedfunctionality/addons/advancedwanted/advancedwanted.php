...
function af_wanted_entry(int $id): array {
 ...
 $q=$db->write_query('SELECT e.*,u.username,ru.username reserved_name FROM '.TABLE_PREFIX.AF_WANTED_ENTRIES.' e LEFT JOIN '.TABLE_PREFIX.'users u ON u.uid=e.author_uid LEFT JOIN '.TABLE_PREFIX.'users ru ON ru.uid=e.reserved_by_uid WHERE e.id='.$id.' LIMIT 1');
...
function af_wanted_render_page(): void {
 ...
 } elseif($action==='view'){if(!$entry)error('Wanted не найден.');$fields=af_wanted_fields();$vals=af_wanted_values([$id])[$id]??[];$byKey=[];foreach($fields as $f)$byKey[$f['field_key']]=$vals[(int)$f['id']]??'';$body.='<h2>Wanted #'.$id.'</h2><p><span class="af-wanted-status">'.af_wanted_status_label($entry['status']).'</span></p><p>Автор: '.build_profile_link(af_wanted_h((string)$entry['username']),(int)$entry['author_uid']).'</p>';if($entry['status']==='reserved')$body.='<p>Придержано пользователем '.build_profile_link(af_wanted_h((string)$entry['reserved_name']),(int)$entry['reserved_by_uid']).'</p>';$body.='<dl>';foreach($fields as $f)if(($f['settings']['show_detail']??true)&&isset($vals[$f['id']])&&$vals[$f['id']]!=='')$body.='<dt>'.af_wanted_h($f['title']).'</dt><dd>'.nl2br(af_wanted_h(af_wanted_display_value($f,$vals[$f['id']],$byKey))).'</dd>';$body.='</dl>'.af_wanted_actions($entry);

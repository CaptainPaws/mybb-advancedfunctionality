...
$checks = [
    ...
    'reserve acquisition is atomic' => strpos($core, "WHERE id=\$id AND status='open'") !== false
    ...
    'reserved user profile is rendered on cards and detail' => substr_count($core, 'Придержано пользователем') >= 2
        && strpos($core, 'ru.username reserved_name') !== false,
    'open normalization clears all lifecycle relations' => strpos($core, "'reserved_by_uid'=>null,'reserved_at'=>0,'application_tid'=>null,'accepted_uid'=>null,'archived_at'=>0") !== false,
    'archive normalization clears incompatible relations' => strpos($core, "'status'=>'archived','reserved_by_uid'=>null,'reserved_at'=>0,'application_tid'=>null") !== false,
